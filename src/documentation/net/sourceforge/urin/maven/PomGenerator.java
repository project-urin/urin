...
package net.sourceforge.urin.maven;
...
public class PomGenerator {
    ...
    private static Tag pom(final String version) {
        return pomTag(tagName("project"),
                ...
                pomTag(tagName("groupId"), text("net.sourceforge.urin")),
                pomTag(tagName("artifactId"), text("urin")),
                pomTag(tagName("version"), text(version)),
                pomTag(tagName("packaging"), text("jar")),
                pomTag(tagName("name"), text("Urin")),
                pomTag(tagName("description"), text("Urin is an open source URI generator and parser written in Java.")),
                pomTag(tagName("url"), text("http://urin.sourceforge.net")),
                ...
                pomTag(tagName("scm"),
                        pomTag(tagName("url"), text("git://git.code.sf.net/p/urin/code"))
                ),
    ...
    private static void writeXml(final Tag root, final File destination, final String fileName) throws IOException, XmlWriteException {
        ...
        new PrettyXmlFormatter().write(Document.document(root), fileWriter);
        fileWriter.close();
    }

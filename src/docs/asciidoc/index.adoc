...
Urin is a URI generator and parser for Java. It is written to make the dynamic generation of correct URIs easier than with Java's built-in URI and URL classes, and to provide support for the current URI standard, https://tools.ietf.org/html/rfc3986[RFC 3986].  Urin has no dependencies.  It's licensed under the http://www.apache.org/licenses/LICENSE-2.0[Apache 2 License].
...
The model of URIs and relative references in Urin reflects that defined in http://tools.ietf.org/html/rfc3986[RFC 3986]. The RFC defines two top level structures:
...
The `uri` variable contains ``{empty}
include::{docfile}/../../java/net/sourceforge/urin/ExamplesTest.java[tags=generatedFtpUri]
``.
...
This produces ``{empty}
include::{docfile}/../../java/net/sourceforge/urin/ExamplesTest.java[tags=generatedMailtoUri]
``. Note, however, that `java.net.URI` implements the obsoleted https://tools.ietf.org/html/rfc2396[RFC 2396], meaning there are certain valid URIs which can be produced using Urin, but which can't be represented by `java.net.URI`.
...
This returns a String containing ``{empty}
include::{docfile}/../../java/net/sourceforge/urin/ExamplesTest.java[tags=generatedRsyncRelativeReference]
``.  It is possible to retrieve this as a `java.net.URI` in the same way as for a `net.sourceforge.urin.Urin`, by calling the `asUri()` method. Of note in this example:
...
The `uri` variable contains ``{empty}
include::{docfile}/../../java/net/sourceforge/urin/ExamplesTest.java[tags=generatedFullFeaturedHttpUri]
...
=== Normalisation
RFC 3986 specifies a number of https://datatracker.ietf.org/doc/html/rfc3986#section-6[methods of URI normalisation] which are applied by Urin, such as the handling of unnecessary encoding and non-preferred case, and the handling of `.` and `..` segments. For example:
...
The `uri` variable contains ``{empty}
include::{docfile}/../../java/net/sourceforge/urin/ExamplesTest.java[tags=normalisedUri]
...
----
This returns the String ``{empty}
include::{docfile}/../../java/net/sourceforge/urin/ExamplesTest.java[tags=resolvedRelativeReference]
``.
